...
attitudes <- train %>% group_by(treatment) %>%
  summarize(avg_start = mean(att_start),
...
```{r 8b}
attitudes2 <- attitudes %>% 
  pivot_wider(id_cols = c(avg_att_chg, treatment), names_from = treatment, values_from = avg_att_chg) %>%
...
The units would be the people in the study.
The treatments is being exposed to a Spanish speaker.
The outcomes are the attitude change.
The causal effect: to determine whether exposure to a Spanish speaker changes the attitude change. 
The fundamental problem of causal inferenece is that you can't observe two simultaneous results for the same unit. 
...
means <- train %>%
  group_by(treatment) %>%
  summarize(avg_change = mean(att_chg)) %>%
  pull(avg_change)
...
graph <- train %>%
  ...
  geom_hline(yintercept = means[1], color = "red") +
  geom_hline(yintercept = means[2], color = "blue") +
  theme_classic()
...
```{r scene12}
graph <- train %>%
  mutate(treatment_numeric = if_else(treatment == "Treated", 1, 0)) %>%
  ggplot(aes(treatment_numeric, att_chg)) +
  geom_point() +
  geom_jitter(width = .25, height = 0) +
